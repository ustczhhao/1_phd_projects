...
class Populations(object):
    ...
    def make_gametes(self, parents):
        ...
        return gametes_ctrl_upper, gametes_ctrl_lower
    ...
    def make_zygotes(self, random_mating=False):
        ...
        if random_mating == False:
            ...
            total_parents = [first_parents, second_parents]
            selected_index = np.random.randint(2, size =1)[0]
            parents = total_parents[selected_index]


        zy_bene = self.soma_bene[rReps, parents,]
        zy_dele = self.soma_dele[rReps, parents,]


        these_gametes_ctrl_upper = these[0]
        these_gametes_ctrl_lower = these[1]

        those_gametes_ctrl_upper = those[0]
        those_gametes_ctrl_lower = those[1]
...
amito_50.simulateNsave('Fit_RME100M_P2SN_N2K_Bene01_MuEvo_OnlyModRecomine.csv', 20*1000, model ='WF', asex_type='mitosis',sex_freq=100,random_mating=True,strides=1)


save_object(amito_50.total_sm_mu_dele, 'RME100M_P2SN_N2K_Bene01_Total_SM_MU_Dele_OnlyModRecomine')
save_object(amito_50.total_pop_mfit, 'RME100M_P2SN_N2K_Bene01_Total_Pop_MFit_OnlyModRecomine')

save_object(amito_50.total_fit_var, 'RME100M_P2SN_N2K_Bene01_Total_Pop_Fit_Var_OnlyModRecomine')
save_object(amito_50.total_mu_dele_var, 'RME100M_P2SN_N2K_Bene01_Total_SM_DeleMu_Var_OnlyModRecomine')
save_object(amito_50.total_dele_covariance, 'RME100M_P2SN_N2K_Bene01_Covariance_FitvsDeleMu_OnlyModRecomine')

save_object(amito_50.sm_ctrl_upper, 'RME100M_P2SN_N2K_Bene01_SM_Ctrl_Upper_OnlyModRecomine')
save_object(amito_50.sm_ctrl_lower, 'RME100M_P2SN_N2K_Bene01_SM_Ctrl_Lower_OnlyModRecomine')
